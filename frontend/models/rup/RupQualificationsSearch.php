<?php

namespace frontend\models\rup;

use yii\base\Model;
use yii\data\ActiveDataProvider;
use frontend\models\rup\RupQualifications;

/**
 * RupQualificationsSearch represents the model behind the search form of `frontend\models\rup\RupQualifications`.
 */
class RupQualificationsSearch extends RupQualifications
{
    /**
     * {@inheritdoc}
     */
    public function rules()
    {
        return [
            [['id', 'time_years', 'time_months', 'rup_id', 'qualification_code'], 'integer'],
            [['qualification_name'], 'safe'],
        ];
    }

    /**
     * {@inheritdoc}
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = RupQualifications::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'id' => $this->id,
            'time_years' => $this->time_years,
            'time_months' => $this->time_months,
            'rup_id' => $this->rup_id,
            'qualification_code' => $this->qualification_code,
        ]);

        $query->andFilterWhere(['ilike', 'qualification_name', $this->qualification_name]);

        return $dataProvider;
    }
}
